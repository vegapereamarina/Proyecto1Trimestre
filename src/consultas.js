...
db.musica.find({"realizado": {$gt:2000}})

//toda la música que no hubiese sido realizada en el 1998 con el genero de música clásica.
db.musica.find({$and: [ {realizado: {$eq: 1998}} , {genero:{$ne: "música clásica"}} ]})
...
db.musica.find({$and: [ {realizado:{$gte:1997 , $lte:2005}} , {unidadesVendidas:{$gt: 18}} ] })

//Toda la música que esté entre los años 1960 y 1970, o los que tengan mas de 20 unidades vendidas
db.musica.find({$or: [ {realizado:{$gte:1960 , $lte:1970}} , {unidadesVendidas:{$gt: 20}} ] })

//toda la música que no haya sido creada antes del 2000, o que no tenga "o"
db.musica.find({$nor: [{realizado:{$lt:2000}} , {album: {$regex:/o$/}}]})
...
//Toda la música que no está en la decada de los 90s, solo tenga como integrantes la orquesta, no haya ganado un grammy y el autor haya nacido despues de los 70s
db.musica.find({$and: [{"datos.realizado":{$not:/199$/}}, {"integrantes":"orquesta"}, {grammy: false}, {nacimiento: {$gt: new Date("1970:01:01")}}]})
